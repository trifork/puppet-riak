class riak(
    ...
    $package_name = "riak-${version}_amd64.deb"
    $package_location = "/tmp/${riak_package_name}"
    ...
    exec {"download-riak":
        command => "wget -o ${package_location} ${download_url}",
        unless => "echo ```curl -s ${download_url}.sha``` \\*${package_location} | sha1sum -c -",
    }
